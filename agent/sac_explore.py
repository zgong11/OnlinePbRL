...
class SACAgent_Explore(Agent):
    ...
    def save_last(self, model_dir):
        torch.save(self.actor.state_dict(), '%s/actor_last.pt' % (model_dir))
        torch.save(self.critic.state_dict(), '%s/critic_last.pt' % (model_dir))
        torch.save(self.critic_target.state_dict(), '%s/critic_target_last.pt' % (model_dir))
    
    def load_last(self, model_dir):
        self.actor.load_state_dict(torch.load('%s/actor_last.pt' % (model_dir), map_location=torch.device('cpu')))
        
    def load_actor(self, model_dir):
        self.actor.load_state_dict(torch.load('%s/actor.pt' % (model_dir), map_location=torch.device('cpu')))
    ...
    def load_actor_unsuper(self, model_dir):
        self.actor.load_state_dict(torch.load('%s/actor_unsup.pt' % (model_dir), map_location=torch.device('cpu')))
    
    def save_unsup(self, model_dir):
        torch.save(self.actor.state_dict(), '%s/actor_unsup.pt' % (model_dir))
        torch.save(self.critic.state_dict(), '%s/critic_unsup.pt' % (model_dir))
        torch.save(self.critic_target.state_dict(), '%s/critic_target_unsup.pt' % (model_dir))
    ...
    def update(self, replay_buffer, logger, step, gradient_update=1):
        for index in range(gradient_update):
            obs, action, ext_rewards, int_rewards, next_obs, not_done, not_done_no_max = replay_buffer.sample(self.batch_size)
            ...
            self.update_critic(obs, action, ext_rewards, int_rewards, next_obs, not_done_no_max, logger, step, print_flag)
    ...
    def update_apt(self, replay_buffer, logger, step, gradient_update=1):
        for index in range(gradient_update):
            obs, full_obs, action, reward, next_obs, not_done, not_done_no_max = replay_buffer.sample_state_ent_batch(self.batch_size)
            ...
            self.update_critic_apt(obs, full_obs, action, reward, next_obs, not_done_no_max, logger, step, print_flag)
